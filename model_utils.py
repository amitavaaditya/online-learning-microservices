...
def predict(df):
    ...
    model = rebuild_model_with_df(df)
    prediction = list(model.predict(lambda: input_fn(df)))
    class_labels = {
        0: 'no.',
        1: 'yes.'
    }
    return {
        'probabilities': dict(zip(class_labels.values(),
                                  prediction[0]['probabilities'].tolist())),
        'class_id': int(prediction[0]['class_ids'][0]),
        'class_label': class_labels[prediction[0]['class_ids'][0]]
    }
